...
function showDays(monthIndex: number) {
...
function addDate(year: number, monthIndex: number, dayNumber: number) {
  const title = getTargetElement('title', document.getElementsByTagName('h2'));
  if (title) title.innerText = `Your timeline of affairs for ${months[monthIndex]} ${dayNumber}, ${year}`;
  removeModal();
}
